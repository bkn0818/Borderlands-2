...
void CameraCtrl::SetupCamera(LPDIRECT3DDEVICE9 pDevice, DWORD zH)
{
	...
	//뷰행렬 설정
	D3DXVECTOR3 vEye(0.0f, 200.0f, -(float)zH);
	...
	//프로젝션 행렬 설정
	D3DXMatrixPerspectiveFovLH(&projMatrix, D3DX_PI / 4, 1.0f, 1.0f, 10000.0f);
...
void CameraCtrl::ProcessMouse(LPDIRECT3DDEVICE9 pDevice, DWORD& x, DWORD& y, HWND hWnd)
{
	POINT pt;
	...
	pDevice->SetTransform(D3DTS_VIEW, &viewMatrix); //카메라 행렬 설정

													//마우스를 윈도우의 중앙으로 초기화 (에임)
	RECT rect;
	GetClientRect(hWnd, &rect);
	pt.x = (int)(rect.right - rect.left) / 2;
	pt.y = (int)(rect.bottom - rect.top) / 2;
