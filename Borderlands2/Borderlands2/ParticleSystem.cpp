...
bool ParticleSystem::Init(char * fileName)
{
	...
	if (FAILED(hr))
	{
		MessageBox(g_hWnd, L"Failed to create vertex buffer", L"Particle", 0);
		return false;
	}
	...
	if (tex == nullptr)
	{
		MessageBox(g_hWnd, L"Failed to Create Texture", L"texture", 0);
		return false;
	}
	return true;
}
...
void ParticleSystem::PreRender()
{
	...
	//거리에 따른 파티클의 크기 제어
	D3DDEVICE->SetRenderState(D3DRS_POINTSCALE_A, FloatToDWORD(3.0f));
	D3DDEVICE->SetRenderState(D3DRS_POINTSCALE_B, FloatToDWORD(2.0f));
